...
namespace ON.interaction { 
    public class Interactable_Reticle : Interactable
    {
        ...
        public override void HandleHover()
        {
            ...
            if (!interactableTriggered && hoverCounter >= options.hoverTime)
            {
            ...
            if (hoverCounter < options.hoverTime)
            {
                ...
                thisReticle.transform.GetChild(0).GetComponent<MeshRenderer>().material.SetFloat("_Percent", swap ? hoverCounter / options.hoverTime : 1 - (hoverCounter / options.hoverTime));
